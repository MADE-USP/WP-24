...

{
  ...
  #Diretório
  setwd("G:\\Meu Drive\\Documentos\\Projetos\\Made\\Projeto inflação\\1. Inflação por grupos IPCA")
...
{
  #Tabelas 7060/1419/2938 IPCA com pesos (2 dígitos)
  link_ipca_pesos_2d_2020 <- "/t/7060/n1/all/v/63,66/p/all/c315/7169,7170,7171,7432,7445,7446,7479,7486,7487,7521,7548,7558,7559,7604,7615,7620,7625,7626,7660,7661,7683,7697,7712,7713,7730,7766,7767,7786,7787/d/v63%202,v66%204"
  link_ipca_pesos_2d_2012 <- "/t/1419/n1/all/v/63,66/p/all/c315/7169,7170,7171,7432,7445,7446,7479,7486,7487,7521,7548,7558,7559,7604,7615,7620,7625,7626,7660,7661,7683,7697,7712,7713,7730,7766,7767,7786,7787/d/v63%202,v66%204"
  link_ipca_pesos_2d_2006 <- "/t/2938/n1/all/v/63,66/p/all/c315/7169,7170,7171,7432,7445,7446,7479,7486,7487,7521,7548,7558,7559,7604,7615,7620,7625,7626,7660,7661,7683,7697,7712,7713,7730,7766,7767,7786,7787/d/v63%202,v66%204"
}

{
  #Categorias de 2 dígitos
  df_ipca_2d_2020 <-as.data.frame(get_sidra(api = link_ipca_pesos_2d_2020))
  df_ipca_2d_2012 <-as.data.frame(get_sidra(api = link_ipca_pesos_2d_2012))
  df_ipca_2d_2006 <-as.data.frame(get_sidra(api = link_ipca_pesos_2d_2006))
}

{
  #Empilhando os dados
  df_ipca_2d <- df_ipca_2d_2020 %>%
    bind_rows(df_ipca_2d_2012,df_ipca_2d_2006)
}

{
  #Dataframe da contribuição dos componentes do IPCA no índice geral
  df_ipca_2d["Data"] <- as.Date(paste(substr(df_ipca_2d$`Mês (Código)`,0,4),substr(df_ipca_2d$`Mês (Código)`,5,6),"01",sep = "-"), format="%Y-%m-%d")
  df_ipca_2d <- dplyr::select(df_ipca_2d,!c(1:4,6:8,10:11))
  df_ipca_2d <- spread(df_ipca_2d,"Variável", value = "Valor") 
  df_ipca_2d["Contribuição"] <- df_ipca_2d$`IPCA - Peso mensal`/100 *df_ipca_2d$`IPCA - Variação mensal`
  df_ipca_2d <- df_ipca_2d %>%
    dplyr::select(Data,"Geral, grupo, subgrupo, item e subitem", "Geral, grupo, subgrupo, item e subitem (Código)", "IPCA - Variação mensal", "IPCA - Peso mensal", "Contribuição") %>%
    filter(Data >= '2007-01-01' & Data <= '2024-12-01')
  View(df_ipca_2d)
}

{
  ...
  dados_ipca <- 
    df_ipca_2d %>%
    dplyr::select(Data,"Geral, grupo, subgrupo, item e subitem (Código)", "IPCA - Variação mensal") %>%
    group_by(Data) %>%
    pivot_wider(names_from = "Geral, grupo, subgrupo, item e subitem (Código)", 
                values_from = "IPCA - Variação mensal") %>%
    ungroup() %>%
    dplyr::select(Data, "7169", everything())
  View(dados_ipca)
}

# Importar dados do BC ---------------------------------------------------------
{
  dados_bc <- GetBCBData::gbcbd_get_series(
    id = c("commodity" = 27574,            # IC-Br - Índice de Commodities do Brasil
           "ipca_adm" = 4449,              # IPCA - Preços administrados
           "cambio" = 3698,                # Taxa de câmbio - R$ / US$ - comercial (compra) - média mensal
           "ulc" = 11777,                  # Custo unitário do trabalho
           "ibc" = 24363),                 # IBC-Br - Índice de Atividade Econômica do BC
    first.date = "2007-01-01",
    last.date = "2024-12-01",
    format.data = "wide")
  
  names(dados_bc)[names(dados_bc) == 'ref.date'] <- 'Data'
  dados_bc["Data"] <- as.Date(dados_bc$Data, format = "%Y-%m-%d")
  
  View(dados_bc)
}

dados_bc_selic1 <- GetBCBData::gbcbd_get_series(
  id = c("selic" = 432                     #| Taxa de juros - Meta Selic definida pelo Copom - Diária
  ),            
  first.date = "2007-01-01",
  ...
  group_by(month, year) |>
  summarise(selic = round(mean(selic),2)) |>
  arrange(year, month, selic) |>
  unite(col = 'data', year, month, sep = '-') |>
  mutate(date = ymd(paste(data, '01', sep = '-'))) |>
  dplyr::select(date, selic)

dados_bc_selic2 <- GetBCBData::gbcbd_get_series(
  id = c("selic" = 432                     #| Taxa de juros - Meta Selic definida pelo Copom - Diária
  ),            
...
dados_bc_selic3 <- GetBCBData::gbcbd_get_series(
  id = c("selic" = 432                     #| Taxa de juros - Meta Selic definida pelo Copom - Diária
  ),            
...
dados_bc_selic <- rbind(dados_bc_selic1,dados_bc_selic2,dados_bc_selic3) |>
  rename(Data = "date")
View(dados_bc_selic)

# Gerar dmmy de pandemia -------------------------------------------------------

{
  # Gerar dummy de pandemia
  d <- data.frame(Data = seq(from = as.Date("2007-01-01"), by = "month", length.out = nrow(dados_bc_selic)),
                  d = c(ifelse(dados_bc_selic$Data >= '2020-03-01' & dados_bc_selic$Data < '2022-03-01',1,0)))
  View(d)
}
...
{
  # Base de dados para dessazonalizar
  df_list <- list(dados_bc, dados_bc_selic, dados_ipca)
  
  dados <-
    df_list %>%
    reduce(full_join, by = c("Data"))
  
  dados <- ts(dados,start=c(2007,01),end=c(2024,12),freq=12)
  dados <- dados[,-1]
  View(dados)
}

# Dessazonalizar todas as colunas
dados.s <- dados
for (i in 1:ncol(dados.s)) {
  dados.s[,i] <- final(seas(dados.s[,i]))
}
View(dados.s)

# Gerar hiato do produto
hp <- hpfilter(dados.s[,"ibc"], type='lambda', freq=14400)  # Filtrando IBC-Br
ibc.gap <- 100*(hp$cycle / hp$trend)  # Criando o gap do produto
ts.plot(ibc.gap)

# Criar o data frame
data <- data.frame(dados.s, ibc.gap = c(ibc.gap), d = c(d[,2]))
View(data)

names(data) <- gsub("\\X", "p", names(data))
View(data)
...
{
  data_7169 <- dados |>
    dplyr::select(p7169, selic)
  
  exog_data <- dados %>%
    dplyr::select(cambio, ulc, ibc.gap, commodity, ipca_adm)
}
...
modelo_7169 <- lp_lin(data_7169,
                      ...
                      hor            = 24)
#plot(modelo_7169)
#plot_lin(modelo_7169)[[2]]

plot_7169 <- 
  data.frame(h = 0:24,
             mean = modelo_7169$irf_lin_mean[1,,2],
             low = modelo_7169$irf_lin_low[1,,2],
             up = modelo_7169$irf_lin_up[1,,2]) %>% 
  ggplot() +
  ...
  labs(title = "Overall inflation",
       subtitle = "(Response to an increase in the Selic rate, in p.p)") +
  ylab("")+
  xlab("") +
...
plot_7170 <- 
  data.frame(h = 0:24,
             ...
             meanb = modelo_7169$irf_lin_mean[1,,2],
             lowb = modelo_7169$irf_lin_low[1,,2],
             upb = modelo_7169$irf_lin_up[1,,2]) %>% 
  ggplot() +
  ...
  theme_bw() +
  labs(title = "1. Food and Beverages inflation",
       subtitle = "(Response to an increase in the Selic rate, in p.p)") +
  ylab("")+
  xlab("") +
  theme(plot.title = element_text(size = 12, hjust=0, face = "bold"),
...
plot_7171 <- 
  ...
  geom_hline(yintercept = 0, colour = "black", linetype = "dashed") +
  theme_bw() +
  labs(title = "1.1 Food at home inflation",
       subtitle = "(Response to an increase in the Selic rate, in p.p)") +
  ylab("")+
  xlab("") +
  theme(plot.title = element_text(size = 12, hjust=0, face = "bold"),
...
plot_7432 <- 
  ...
  geom_hline(yintercept = 0, colour = "black", linetype = "dashed") +
  theme_bw() +
  labs(title = "1.2 Food away from home inflation",
       subtitle = "(Response to an increase in the Selic rate, in p.p)") +
  ylab("")+
  xlab("") +
  theme(plot.title = element_text(size = 12, hjust=0, face = "bold"),
...
plot_7445 <- 
  ...
  geom_hline(yintercept = 0, colour = "black", linetype = "dashed") +
  theme_bw() +
  labs(title = "2. Housing inflation",
       subtitle = "(Response to an increase in the Selic rate, in p.p)") +
  ylab("")+
  xlab("") +
  theme(plot.title = element_text(size = 12, hjust=0, face = "bold"),
...
plot_7446 <- 
  ...
  geom_hline(yintercept = 0, colour = "black", linetype = "dashed") +
  theme_bw() +
  labs(title = "2.1 Housing-related expenses and maintenance inflation",
       subtitle = "(Response to an increase in the Selic rate, in p.p)") +
  ylab("")+
  xlab("") +
  theme(plot.title = element_text(size = 12, hjust=0, face = "bold"),
...
plot_7479 <- 
  ...
  geom_hline(yintercept = 0, colour = "black", linetype = "dashed") +
  theme_bw() +
  labs(title = "2.2 Fuel and energy inflation",
       subtitle = "(Response to an increase in the Selic rate, in p.p)") +
  ylab("")+
  xlab("") +
  theme(plot.title = element_text(size = 12, hjust=0, face = "bold"),
...
plot_7486 <- 
  ...
  geom_hline(yintercept = 0, colour = "black", linetype = "dashed") +
  theme_bw() +
  labs(title = "3. Household items inflation",
       subtitle = "(Response to an increase in the Selic rate, in p.p)") +
  ylab("")+
  xlab("") +
  theme(plot.title = element_text(size = 12, hjust=0, face = "bold"),
...
plot_7487 <- 
  ...
  geom_hline(yintercept = 0, colour = "black", linetype = "dashed") +
  theme_bw() +
  labs(title = "3.1 Furniture and utensils inflation",
       subtitle = "(Response to an increase in the Selic rate, in p.p)") +
  ylab("")+
  xlab("") +
  theme(plot.title = element_text(size = 12, hjust=0, face = "bold"),
...
plot_7521 <- 
  ...
  geom_hline(yintercept = 0, colour = "black", linetype = "dashed") +
  theme_bw() +
  labs(title = "3.2 Electronic devices inflation",
       subtitle = "(Response to an increase in the Selic rate, in p.p)") +
  ylab("")+
  xlab("") +
  theme(plot.title = element_text(size = 12, hjust=0, face = "bold"),
...
plot_7548 <- 
  ...
  geom_hline(yintercept = 0, colour = "black", linetype = "dashed") +
  theme_bw() +
  labs(title = "3.3 Repairs and maintenance inflation",
       subtitle = "(Response to an increase in the Selic rate, in p.p)") +
  ylab("")+
  xlab("") +
  theme(plot.title = element_text(size = 12, hjust=0, face = "bold"),
...
plot_7558 <- 
  ...
  geom_hline(yintercept = 0, colour = "black", linetype = "dashed") +
  theme_bw() +
  labs(title = "4. Clothing inflation",
       subtitle = "(Response to an increase in the Selic rate, in p.p)") +
  ylab("")+
  xlab("") +
  theme(plot.title = element_text(size = 12, hjust=0, face = "bold"),
...
plot_7559 <- 
  ...
  geom_hline(yintercept = 0, colour = "black", linetype = "dashed") +
  theme_bw() +
  labs(title = "4.1 Clothing inflation",
       subtitle = "(Response to an increase in the Selic rate, in p.p)") +
  ylab("")+
  xlab("") +
  theme(plot.title = element_text(size = 12, hjust=0, face = "bold"),
...
plot_7604 <- 
  ...
  geom_hline(yintercept = 0, colour = "black", linetype = "dashed") +
  theme_bw() +
  labs(title = "4.2 Footwear and accessories inflation",
       subtitle = "(Response to an increase in the Selic rate, in p.p)") +
  ylab("")+
  xlab("") +
  theme(plot.title = element_text(size = 12, hjust=0, face = "bold"),
...
plot_7615 <- 
  ...
  geom_hline(yintercept = 0, colour = "black", linetype = "dashed") +
  theme_bw() +
  labs(title = "4.3 Jewelry and costume jewelry inflation",
       subtitle = "(Response to an increase in the Selic rate, in p.p)") +
  ylab("")+
  xlab("") +
  theme(plot.title = element_text(size = 12, hjust=0, face = "bold"),
...
plot_7620 <- 
  ...
  geom_hline(yintercept = 0, colour = "black", linetype = "dashed") +
  theme_bw() +
  labs(title = "4.4 Fabrics and notions inflation",
       subtitle = "(Response to an increase in the Selic rate, in p.p)") +
  ylab("")+
  xlab("") +
  theme(plot.title = element_text(size = 12, hjust=0, face = "bold"),
...
plot_7625 <- 
  ...
  geom_hline(yintercept = 0, colour = "black", linetype = "dashed") +
  theme_bw() +
  labs(title = "5. Transportation inflation",
       subtitle = "(Response to an increase in the Selic rate, in p.p)") +
  ylab("")+
  xlab("") +
  theme(plot.title = element_text(size = 12, hjust=0, face = "bold"),
...
plot_7660 <- 
  ...
  geom_hline(yintercept = 0, colour = "black", linetype = "dashed") +
  theme_bw() +
  labs(title = "6. Health and personal care inflation",
       subtitle = "(Response to an increase in the Selic rate, in p.p)") +
  ylab("")+
  xlab("") +
  theme(plot.title = element_text(size = 12, hjust=0, face = "bold"),
...
plot_7661 <- 
  ...
  geom_hline(yintercept = 0, colour = "black", linetype = "dashed") +
  theme_bw() +
  labs(title = "6.1 Pharmaceutical and optical products inflation",
       subtitle = "(Response to an increase in the Selic rate, in p.p)") +
  ylab("")+
  xlab("") +
  theme(plot.title = element_text(size = 12, hjust=0, face = "bold"),
...
plot_7683 <- 
  ...
  geom_hline(yintercept = 0, colour = "black", linetype = "dashed") +
  theme_bw() +
  labs(title = "6.2 Health services inflation",
       subtitle = "(Response to an increase in the Selic rate, in p.p)") +
  ylab("")+
  xlab("") +
  theme(plot.title = element_text(size = 12, hjust=0, face = "bold"),
...
plot_7697 <- 
  ...
  geom_hline(yintercept = 0, colour = "black", linetype = "dashed") +
  theme_bw() +
  labs(title = "6.3 Personal care inflation",
       subtitle = "(Response to an increase in the Selic rate, in p.p)") +
  ylab("")+
  xlab("") +
  theme(plot.title = element_text(size = 12, hjust=0, face = "bold"),
...
plot_7712 <- 
  ...
  geom_hline(yintercept = 0, colour = "black", linetype = "dashed") +
  theme_bw() +
  labs(title = "7. Personal expenses inflation",
       subtitle = "(Response to an increase in the Selic rate, in p.p)") +
  ylab("")+
  xlab("") +
  theme(plot.title = element_text(size = 12, hjust=0, face = "bold"),
...
plot_7713 <- 
  ...
  geom_hline(yintercept = 0, colour = "black", linetype = "dashed") +
  theme_bw() +
  labs(title = "7.1 Personal services inflation",
       subtitle = "(Response to an increase in the Selic rate, in p.p)") +
  ylab("")+
  xlab("") +
  theme(plot.title = element_text(size = 12, hjust=0, face = "bold"),
...
plot_7730 <- 
  ...
  geom_hline(yintercept = 0, colour = "black", linetype = "dashed") +
  theme_bw() +
  labs(title = "7.201 Recreation inflation",
       subtitle = "(Response to an increase in the Selic rate, in p.p)") +
  ylab("")+
  xlab("") +
  theme(plot.title = element_text(size = 12, hjust=0, face = "bold"),
...
plot_7766 <- 
  ...
  geom_hline(yintercept = 0, colour = "black", linetype = "dashed") +
  theme_bw() +
  labs(title = "8. Education inflation",
       subtitle = "(Response to an increase in the Selic rate, in p.p)") +
  ylab("")+
  xlab("") +
  theme(plot.title = element_text(size = 12, hjust=0, face = "bold"),
...
plot_7786 <- 
  ...
  geom_hline(yintercept = 0, colour = "black", linetype = "dashed") +
  theme_bw() +
  labs(title = "9. Communication inflation",
       subtitle = "(Response to an increase in the Selic rate, in p.p)") +
  ylab("")+
  xlab("") +
  theme(plot.title = element_text(size = 12, hjust=0, face = "bold"),
